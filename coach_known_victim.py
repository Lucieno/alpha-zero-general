...
class CoachKnownVictim():
    """
    This class executes the self-play + learning. It uses the functions defined
    in Game and NeuralNet. args are specified in main.py.
    """
    def __init__(self, game, nnet, victim_net, args):
        self.game = game
        self.nnet = nnet
        ...
        for i in range(1, self.args.numIters+1):
            ...
            if not self.skipFirstSelfPlay or i>1:
                ...
                for eps in range(self.args.numEps):
                    self.mcts = MctsKnownVictim(self.game, self.nnet, self.victim_mcts, self.args)   # reset search tree
            ...
            pmcts = MctsKnownVictim(self.game, self.pnet, self.victim_mcts, self.args)

            pitted_victim_mcts = MCTS(self.game, self.victim_net, self.args)
            
            self.nnet.train(trainExamples)
            nmcts = MctsKnownVictim(self.game, self.nnet, self.victim_mcts, self.args)
            ...
            print('Adversary/Victim WINS : %d / %d ; DRAWS : %d' % (nwins, pwins, draws))
